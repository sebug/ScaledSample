...
resource "azurerm_virtual_machine_extension" "kubernetesdev" {
  ...
  settings = <<SETTINGS
{
  "commandToExecute": "PowerShell -NoProfile -ExecutionPolicy Bypass -Command \"iex ((New-Object System.Net.WebClient).DownloadString('https://chocolatey.org/install.ps1'))\" && SET \"PATH=%PATH%;%ALLUSERSPROFILE%\\chocolatey\\bin\" && choco install -y git && mkdir C:\\Tools && mkdir C:\\Sources && cd C:\\Sources && \"C:\\Program Files\\Git\\cmd\\git.exe\" clone https://github.com/sebug/TalkNotesBack.git && \"C:\\Program Files\\Git\\cmd\\git.exe\" clone https://github.com/sebug/TalkNotesFront.git && \"C:\\Program Files\\Git\\cmd\\git.exe\" clone https://github.com/sebug/InvokeDockerServer.git && \"C:\\Program Files\\Git\\cmd\\git.exe\" clone https://github.com/sebug/TalkNotesComposed.git && choco install -y docker && choco install -y docker-compose"
}
SETTINGS
}
